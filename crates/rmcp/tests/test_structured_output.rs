...
use rmcp::{
    ...
    model::{CallToolResult, Tool, Content},
...
impl TestServer {
    ...
    pub async fn get_user(&self, user_id: Parameters<String>) -> Result<Structured<UserInfo>, String> {
        if user_id.0 == "123" {
            Ok(Structured(UserInfo {
...
async fn test_structured_return_conversion() {
    // Test that Structured<T> converts to CallToolResult with structured_content
    let calc_result = CalculationResult {
        sum: 7,
        product: 12,
    };
    
    let structured = Structured(calc_result);
