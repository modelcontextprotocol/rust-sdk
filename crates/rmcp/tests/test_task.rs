...
use axum::serve;
use rmcp::{
    model::{ServerCapabilities, ServerInfo, ServerResult}, task_manager::{AsyncHandler, OperationMessage, OperationProcessor, OperationResultTransport}, ServerHandler, Service
};
...
impl ServerHandler for Server {
    ...
    ) -> impl Future<Output = Result<Option<rmcp::model::ServerResult>, rmcp::ErrorData>> + Send + '_
    {
        ...
        async move {
            let operation_id = ctx_cloned.id.to_string();
            let server_arc = self_cloned.clone();
            ...
            let handler: AsyncHandler = Arc::new(move |op_msg: OperationMessage| {
                ...
                Box::pin(async move {
                    ...
                    server_arc.handle_request(op_msg.client_request, op_msg.context).await;
