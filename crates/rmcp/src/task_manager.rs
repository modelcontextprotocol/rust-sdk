...
use crate::error::RmcpError as Error;

/// Operation message describing a unit of asynchronous work.
#[derive(Debug, Clone)]
pub struct OperationMessage {
    pub operation_id: String,
    /// Arbitrary metadata; should include a key like "handler" to route to the registered async function
...
impl OperationProcessor {
    ...
    pub fn submit_operation(&mut self, message: OperationMessage) -> Result<(), Error> {
        ...
        let handler_name = message.metadata.get("handler").cloned().ok_or_else(|| {
            Error::TaskError("Missing 'handler' metadata for operation".to_string())
        })?;
