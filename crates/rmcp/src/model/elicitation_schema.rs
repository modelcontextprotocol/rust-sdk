...
pub struct TitledItems {
    // Note: Schemars produces "oneOf" here, but MCP spec uses "anyOf"
...
mod tests {
    ...
    #[test]
    fn test_enum_schema_untitled_multi_select_serialization() -> Result<()> {
        let schema = EnumSchema::builder(vec!["US".to_string(), "UK".to_string()])
            .multiselect()
            .min_items(1u64)
            .map_err(|e| anyhow!("{e}"))?
    ...
    #[test]
    fn test_enum_schema_titled_single_select_serialization() -> Result<()> {
        let schema = EnumSchema::builder(vec!["US".to_string(), "UK".to_string()])
            .enum_titles(vec![
    ...
    #[test]
    fn test_enum_schema_titled_multi_select_serialization() -> Result<()> {
        let schema = EnumSchema::builder(vec!["US".to_string(), "UK".to_string()])
            .enum_titles(vec![
                "United States".to_string(),
                "United Kingdom".to_string(),
