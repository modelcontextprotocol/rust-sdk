...
impl AuthorizationManager {
    ...
    pub async fn new<U: IntoUrl>(base_url: U) -> Result<Self, AuthError> {
        Self::with_token_store(base_url, Arc::new(InMemoryTokenStore::new())).await
    }

    /// Create new auth manager with base url and custom token store
    pub async fn with_token_store<U: IntoUrl>(
        base_url: U,
        token_store: Arc<dyn TokenStore>,
